...
class VirtualGateMatrices:
    ...
    def get_virtual_gate_projection(self, physical_channels):
        ...
        if self._projection_cache_physical_channels == list(physical_channels) and len(
            self._virtual_matrices
        ) == len(self._projection_cache_matrices):
    ...
    def add(
        ...
        existing_virtual = []
        for vm in self._virtual_matrices.values():
            existing_virtual.extend(vm.virtual_gates)

        np_matrix = np.array(matrix)
        shape = np_matrix.shape
        n_real, n_virtual = (
            (shape[1], shape[0]) if real2virtual else (shape[0], shape[1])
        )

        if n_real != len(real_gate_names):
            raise ValueError(
                f"size virtual gate matrix ({n_real}) doesn't match "
                f"the number of real gates names({len(real_gate_names)})"
            )
        if n_virtual != len(virtual_gate_names):
            raise ValueError(
                "size virtual gate matrix ({n_virtual}) doesn't match "
        ...
        defined_virtual = existing_virtual
        ...
        if filter_undefined:
            ...
            if len(valid_indices) == 0:
                logger.warning(
                    f"No valid gates found of the AWG for the virtual gate matrix {name}."
            ...
            if len(not_defined_gates):
                logger.warning(
                    f"Gates {not_defined_gates} of virtual gate matrix {name} "
            ...
            for gate in real_gate_names:
                if gate not in defined_channels:
                    raise Exception(
                        f"Gate {gate} of virtual gate matrix {name} is not defined"
                    )

        for gate in virtual_gate_names:
            if gate in defined_virtual:
                raise Exception(
                    f"Gate {gate} of virtual gate matrix {name} already defined"
                )

        vgm = VirtualGateMatrix(
            real_gate_names,
            virtual_gate_names,
            matrix,
